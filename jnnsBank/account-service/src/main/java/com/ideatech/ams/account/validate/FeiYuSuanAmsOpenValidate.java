...
@Slf4j
public class FeiYuSuanAmsOpenValidate extends AbstractAllPublicAccountOpenValidate {
    ...
    protected void doValidate(AllBillsPublicDTO dto) {
        ...
        if (StringUtils.isEmpty(dto.getAccountKey())) {
        ...
        if (StringUtils.isEmpty(dto.getAcctFileType())) {
            throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR, "AMSERROR-0056:开户证明文件1类型不能为空");
        }
        if (StringUtils.isNotEmpty(dto.getAcctFileType()) && (!"10".equals(dto.getAcctFileType()))) {
            if (StringUtils.isEmpty(dto.getAcctFileNo())) {
        ...
        if (dto.getAccountNameFrom().equals("2")) {
            if (StringUtils.isEmpty(dto.getSaccpostfix())) {
                throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR, "AMSERROR-0055:请输入账户后缀！");
            }
            if (StringUtils.isNotEmpty(dto.getSaccpostfix())) {
                try {
                    if (dto.getSaccpostfix().getBytes("GBK").length > 42) {
                        throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR,"账户后缀账户不能大于42个字符或21个汉字,请重新录入!");
                    }
                } catch (Exception e) {
                    throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR,"账户后缀账户不能大于42个字符或21个汉字,请重新录入!");
                }
        ...
        if (dto.getAccountNameFrom().equals("1")) {
            if (RegexUtils.length(dto.getInsideDeptName()) < 0
                    || RegexUtils.length(dto.getInsideDeptName()) > 70) {
...
//      if (StringUtils.isEmpty(dto.getInsideLeadName())) {
...
//        throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR,"AMSERROR-0071:内设部门联系电话不能为空!");
//      }
...
//      }
            ...
            if(StringUtils.isNotBlank(dto.getFundManager())){
                if (RegexUtils.length(dto.getFundManager()) < 0 || RegexUtils.length(dto.getFundManager()) > 70) {
                    throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR, "AMSERROR-0072:资金管理人姓名超长(70个字符或35个汉字),请重新录入!");
                }
            }
        ...
        if (com.ideatech.common.utils.StringUtils.isNotBlank(dto.getAcctFileType())) {
            ...
            if (!ArrayUtils.contains(accountFileType, dto.getAcctFileType())) {
                log.info("非预算户证明文件1类型值当前值：" + dto.getAcctFileType());
                log.info("非预算户证明文件1类型值应为：\"09\", \"10\",\"11\"");
                throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR,"非预算户开户证明文件1类型值不正确");
            }
        }
        if(com.ideatech.common.utils.StringUtils.isNotBlank(dto.getAcctFileType2())){
            ...
            if (!ArrayUtils.contains(accountFileType2, dto.getAcctFileType2())) {
                log.info("非预算户证明文件2类型值当前值：" + dto.getAcctFileType2());
                log.info("非预算户证明文件2类型值应为：\"08\"");
                throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR,"非预算户开户证明文件2类型值不正确");
            }
        }
        ...
        if (com.ideatech.common.utils.StringUtils.isBlank(dto.getAcctFileType2()) || com.ideatech.common.utils.StringUtils.isBlank(dto.getAcctFileNo2())) {
            if (com.ideatech.common.utils.StringUtils.isNotEmpty(dto.getAcctFileNo2()) || com.ideatech.common.utils.StringUtils.isNotEmpty(dto.getAcctFileType2())) {
                throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR,"证明文件2编号与证明文件2种类必须全部为空、或全部不为空!");
            }
        }
        ...
        if(com.ideatech.common.utils.StringUtils.isNotBlank(dto.getAccountNameFrom())){
            if (!ArrayUtils.contains(accountNameFrom, dto.getAccountNameFrom())) {
                log.info("非预算户账户名称构成方式类型当前值：" + dto.getAccountNameFrom());
                log.info("非预算户账户名称构成方式类型值应为：\"0\",\"1\",\"2\"");
                throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR,"非预算户账户名称构成方式类型值不正确");
            }
        }
        ...
        if(com.ideatech.common.utils.StringUtils.isNotBlank(dto.getCapitalProperty())){
            if (!ArrayUtils.contains(capitalProperty, dto.getCapitalProperty())) {
                log.info("非预算户资金性质类型当前值：" + dto.getCapitalProperty());
                log.info("非预算户资金性质类型值应为：\"01\",\"02\",\"03\",\"04\",\"05\",\"06\",\"07\",\"08\",\"09\",\"10\",\"11\",\"12\",\"13\",\"14\",\"16\"");
                throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR,"非预算户资金性质类型值不正确");
            }
        }
