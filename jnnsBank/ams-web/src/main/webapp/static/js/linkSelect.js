...
layui.define(['layer','form','element','laytpl'], function(exports){
    ...
    //主模板
    var TPL_MAIN = ['<label class="layui-form-label">{{ d.data.lableName }}</label>',
        '<div class="layui-input-block">',
        '<div class="layui-unselect layui-form-select " id="linkSelectDiv{{ d.index }}">',
        '<div class="layui-select-title">',
        '<input type="text" id="linkSelectInput{{ d.index }}" name="linkSelectInput{{ d.index }}" placeholder="{{ d.data.placeholderText }}"',
        '{{# if(d.data.disabled){ }}',
        ' disabled',
        '{{# } }}',
        ' readonly class="layui-input layui-unselect"></input>',
        '{{# if(!d.data.disabled){ }}',
    ...
    //下拉模板
    var TPL_DL = [
        '<dl class="layui-anim layui-anim-upbit" id="linkSelectdl{{ d.sindex }}"' +
        ' style="width:{{ d.options.selectWidth }}px;min-width:160px;left:{{ d.left }}px">',
        '{{# if(d.nodata){  }}',
    ...
    Class.prototype.optionFilter = function(dl){
        ...
        dds.on('click', function(e){
            ...
            var othis = $(this), value = othis.attr('lay-value'),name = othis.text(),level=othis.attr('lay-sindex'),thissIndex = othis.attr('lay-sindex');
            ...
            if(othis.hasClass('layui-select-tips')){
                ...
                othis.addClass(THIS);
                INPUT.val(getText(setValue(valueArr,thissIndex,value,name,level)));
            }
            othis.siblings().removeClass(THIS);
            if(!isC){
                //没有子集
                if(!othis.hasClass('layui-select-tips')) $(DIV).removeClass('layui-form-selected');
            ...
            typeof options.selected === 'function' && options.selected({name:name,value:value,level:level},othis,valueArr);
            return false;
        });

        //保存数据
        function setValue(valueArr,thissIndex,value,name,level){
            valueArr = valueArr||[];
            valueArr.splice(thissIndex,valueArr.length);			//将值缓存置空
            if(value) valueArr[thissIndex] = {"name":name,"value":value,"level":level};
    ...
    Class.prototype.renderData = function(data,parentId){
        ...
        var conf = {
            left:options.selectWidth*(that.sindex),
            options:options,
            sindex:that.sindex,
            index: that.index //索引
        }
