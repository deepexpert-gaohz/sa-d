...
//        }
    ...
    if(operateType == 'ACCT_OPEN' || billType == 'ACCT_OPEN' ||buttonType == 'saicValidate' || buttonType == 'applyValidate'){
        ...
        var validLabelLegal = ["legalType","legalName","legalIdcardType","legalIdcardNo", "nationality"];//法人代表信息
        var validLabelOrg = ["orgSelect"];//组织结构代码证信息
        var validLabelContact = ["telephone","zipcode"];//联系信息
        var docCodeFlag = ["docCode"];//影像
        addValidateFlag(validLabelBase.concat(validLabelCustomer,validLabelLegal,validLabelContact,docCodeFlag));//validLabelOrg 去掉组织机构必填
