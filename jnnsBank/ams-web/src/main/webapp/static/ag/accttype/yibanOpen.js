...
layui.use(['form','murl','saic','account', 'picker', 'linkSelect', 'org', 'industry','common', 'loading', 'laydate', 'imageInfo','laytpl'], function () {
    ...
    if(operateType == 'ACCT_OPEN' || billType == 'ACCT_OPEN' || buttonType == 'saicValidate' || buttonType == 'applyValidate'){
        var validLabelBase = ["basicAcctRegArea","accountKey","acctNo","acctFileType","acctType","bankName","bankCode","acctCreateDate","docCode"];//账户信息 "fileType","fileNo",
...
//  	        checkJiBenParAccountKey:true
    ...
    otherValidate = function(){
    	//开户证明文件种类1为07时，必填属性与编号1同时有同时无
//    	acctFileTypeChange(form,validator,true,["","06"]);
        if(validatorParam != undefined){
            $("#acctFileType").attr("lay-filter","acctFileType");
            form.on('select(acctFileType)',function (data) {
                if(data.value=="06"){
                    addValidate("acctFileNo",{required : true},true);
                }else{
...
//    	parLegalIdcardTypeChange(form,validator);
        ...
        //一般户-人行上报
        if(buttonType == 'sync') isYiBanPbcSyncChange(form,validatorParam);
    ...
    otherInit = function(){
    	...
    	//初始化：开户证明文件种类1为07时，必填属性与编号1同时有同时无
//    	acctFileTypeInit(validator,true,["","06"]);
        if(validatorParam != undefined){
            if($("#acctFileType").val() =="06"){
                addValidate("acctFileNo",{required : true},true);
            }
        }
    ...
    if(billId) {
        $.get (uri + '?id=' + billId, null, function (data) {
            ...
            if (operateType == 'ACCT_CHANGE') {
                $.get('../../validate/changeOpen/customerInfo?accountKey=' + accountKey
            ...
            if(operateType == 'ACCT_CHANGE') {
                ...
                //一般户变更-账户信息，只有开户证明文件种类、开户证明文件种类编号和备注可编辑
    ...
    function initByExistData(data) {
        ...
        console.log("depositorType="+depositorType)
        if(depositorType!=null || depositorType!=""){
    ...
    function submitForm(data, submitOperateType){
        ...
        // data.field.acctType = 'yiban';
        // data.field.billType = 'ACCT_OPEN';
