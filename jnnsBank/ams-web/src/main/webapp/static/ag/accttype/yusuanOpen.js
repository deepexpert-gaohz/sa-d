...
layui.use(['form','murl','saic','account', 'picker', 'linkSelect', 'org', 'industry','common', 'loading', 'imageInfo','laytpl'], function () {
    ...
    if(operateType == 'ACCT_OPEN' || billType == 'ACCT_OPEN'  ||buttonType == 'saicValidate' || buttonType == 'applyValidate'){
        ...
        var validLabelBase = ["basicAcctRegArea","accountKey","acctNo","acctType","bankName","bankCode","acctCreateDate","docCode"];
        addValidateFlag(validLabelBase);
...
//  	        checkJiBenParAccountKey:true
            ...
            /*acctFileNo : {
                shouldAllRequiredOrNot : ["#acctFileType"]
            },*/
            acctFileType2 : {
...
//  	        checkJiBenParAccountKey:true
        ...
        var messages = {
            acctFileType : {
                shouldAllRequiredOrNot : "开户证明文件种类编号1必须同时联动必填"
            },
            acctFileNo : {
                shouldAllRequiredOrNot : "开户证明文件种类1必须同时联动必填"
            },
            acctFileType2 : {
    ...
    if(billType == 'ACCT_CHANGE' || operateType == 'ACCT_CHANGE'){
        addValidateFlag(["acctNo","acctType","bankCode","accountKey","basicAcctRegArea","accountNameFrom"]);
    ...
    otherValidate = function(){
    	...
    	acctFileTypeChange(form,validatorParam,true,["09","11"]);

    	//账户构成方式:
    	//1. 选择“存款人名称一致”时，下方显示“资金管理信息”区块，
    	//2. 选择“存款人名称加内设部门”时，下方显示内设部门信息
    	//3. 选择“存款人名称加资金性质”时，下方前缀、后缀、资金管理人信息
    	accountNameFromChange(form,validatorParam,syncEccs);
...
//    	parLegalIdcardTypeChange(form,validator);
    ...
    otherInit = function(){
    	...
    	acctFileTypeInit(validatorParam,true,["09","11"]);
    ...
    if(billId) {
        $.get (uri + '?id=' + billId, null, function (data) {
            ...
            if(operateType == 'ACCT_CHANGE') {
                ...
                //预算变更-账户信息，账号、账户性质、开户日期和资金性质字段不可编辑
                ...
                /*$("#capitalProperty").attr("disabled",true);
                $("#capitalProperty").addClass("disableElement");*/

                //预算变更-授权经办人，姓名、证件类型、证件号码和到期日字段不可编辑
    ...
    // $.get ('/apply/details?id=' + preOpenAcctId, null, function (data) {
    //     if(data.rel){
    ...
    function initByExistData(data) {
        ...
        $('#acctType').attr("disabled",true);
        //取现标识
        $("#enchashmentType").attr("disabled",true);
        $("#enchashmentType").addClass("disableElement");
