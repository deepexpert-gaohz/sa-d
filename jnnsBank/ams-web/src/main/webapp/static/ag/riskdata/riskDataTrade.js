...
var preDate = new Date(curDate.getTime() - 86400000).Format("YYYY-MM-dd"); //前一天
...
function queryRiskData(row){
    $.get('edit.html', null, function (form) {
        addBoxIndex = layer.open ({
            ...
            async:false,
            success: function () {
                riskDataCheck.init(row.modelId,row.riskPoint,row.riskDate,row.status);
                var form = layui.form;
                form.render();
            }
        });
...
riskdataTrade.init = function () {
    ...
    riskdataTrade.table = $('#' + riskdataTrade.tableId).bootstrapTable({
        url: riskdataTrade.baseUrl + '/queryRiskData', //请求后台的URL（*）
        method: 'get', //请求方式（*）
...
layui.use(['form', 'layedit', 'laydate', 'upload'], function () {
    var form=layui.form;
    riskdataTrade.init();
    $.get("../../model/findTypeNameAsRisk", function (res) {
        if(res.code === 'ACK') {
            var options = '';
    ...
    $('#btn_export').on('click', function () {
        ...
        layer.confirm('确定导出数据吗？', null, function (index) {
            window.location.href=riskdataTrade.baseUrl + "/expRiskDataDeal?startEndTime="+dateStr;
            layer.close(index);
        });
