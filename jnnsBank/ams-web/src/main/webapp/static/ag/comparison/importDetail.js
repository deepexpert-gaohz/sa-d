...
list.queryParams = function (params) {
	if (!params)
		return {
			dataId: $("#dataId").val(),
            collectState: $.trim($("#collectState").val())
		};
...
list.init = function () {
	list.table = $('#' + list.tableId).bootstrapTable({
		url: list.baseUrl + '/importDetails?compareTaskId='+taskId + '&dataSourceId=' + dataSourceId, //请求后台的URL（*）
		method: 'get', //请求方式（*）
...
function getCollectState(value){
	if(value == 'success'){
		return '采集成功';
	}else if(value == 'fail'){
        return '采集失败';
