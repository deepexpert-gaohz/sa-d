...
apply.init = function () {
    ...
    apply.table = $('#' + apply.tableId).bootstrapTable({
        url: apply.baseUrl + '/search/price', //请求后台的URL（*）
        method: 'get', //请求方式（*）
...
layui.use(['form', 'layedit', 'laydate','layer','common'], function () {
    ...
    $('#pdf_btn').on('click', function () {
        // var queryParams ="?acctName="+$.trim($("#acctName").val())
        //     +"&beginDate="+$("#beginDate").val()+"&endDate="+$("#endDate").val()
        //     +"&entname="+ $("#entname").val()+"&proofBankName="+$("#proofBankName").val()
        //     +"&username="+$("#username").val()+"&phone="+$("#phone").val()+"&type="+$("#type").val();
        // //window.open(apply.baseUrl + '/pdf'+queryParams,"_blank");
        // document.getElementById("downloadFrame").src = apply.baseUrl+'/price/pdf'+queryParams;
        var params = '';
        ...
        $("#downloadFrame").prop('src', apply.baseUrl+'/price/pdf?'+params);
        return false;
    });
