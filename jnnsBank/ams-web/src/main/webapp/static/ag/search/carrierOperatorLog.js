...
list.columns = function () {
    return [{
        ...
        formatter: function (value, row, index) {
            return showDataNotNull(value);
        }
    }, {
        field: 'idCardedTypeStr',
        title: '核查人员类型'
    }, {
        field: 'createdDate',
        title: '验证时间',
...
list.queryParams = function (params) {
    if (!params)
        return {
            type: "VERIFY_CARRIER_OPERATOR",
            result: $.trim($("#result").val()),
            idCardedType: $.trim($("#idCardedType").val()),
...
layui.use(['form', 'layedit', 'laydate', 'loading'], function () {
    ...
    //获取运营商校验日志页面中“核查人员类型”枚举
    $.get('../../dictionary/32119222946719/option', function (res) {
        if (res.code === "ACK") {
            for(var i=0;i<res.data.length;i++){
                $('#idCardedType').append("<option value='" + res.data[i].value + "'>" + res.data[i].name + "</option>");
            }
        }
        form.render('select');
    });
