...
apply.queryParams = function (params) {
    ...
    var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
        ...
        beginDateAccept: $.trim($("#beginDateAccept").val()),
        endDateAccept: $.trim($("#endDateAccept").val()),
        accepter: $.trim($("#accepter").val()),
...
layui.use(['form', 'layedit', 'laydate'], function () {
    ...
    //初始化日期控件
    //申请时间
    var laydateApply =  beginEndDateInit(laydate, 'beginDateApply', 'endDateApply', 'YYYY-MM-DD hh:mm:ss', true);
    //接洽时间
    var laydateAccept =  beginEndDateInit(laydate, 'beginDateAccept', 'endDateAccept', 'YYYY-MM-DD hh:mm:ss', true);

    //重置按钮
    $('button[type="reset"]').click(function () {
        for (var i = 0; i < laydateApply.length; i++) {
            delete laydateApply[i].max;
            delete laydateApply[i].min;
        }
        for (var i = 0; i < laydateAccept.length; i++) {
            delete laydateAccept[i].max;
            delete laydateAccept[i].min;
        }
    });
