...
var list = {
    baseUrl: "../../networkVerify/openRevokeState/feedback",
    entity: "openRevokeState",
    tableId: "list",
    toolbarId: "toolbar",
    unique: "id",
    order: "asc",
...
list.queryParams = function (params) {
    if (!params)
        return {
            ...
            acctSts: $.trim($("#acctSts").val()),
            chngDt: $.trim($("#chngDt").val()),
...
layui.use(['form', 'layedit', 'laydate', 'loading', 'laytpl'], function () {
    ...
    $('#btn_add').on('click', function () {
        parent.tab.tabAdd({
            title: "开销户状态反馈申请",
            href: 'networkVerify/openRevokeStateFeedback.html'
        });
    });
    ...
    function showTableHTML(id) {
        $.get(list.baseUrl+'/' + id, function (res) {
            ...
            html += '<tr><td class="td-label" colspan="4" align="center">请求信息</td></tr>'+
                ...
                '<td class="td-label">账户状态标识：</td>\n' +
                '<td class="td-label-right">'+(acctStsSwitch(data.acctSts)||'')+'</td>\n' +
                '</tr>\n' +
                '<tr>\n' +
                '<td class="td-label">变更日期：</td>\n' +
                '<td class="td-label-right">'+(data.chngDt||'')+'</td>\n' +
                '</tr>\n' ;
            ...
            if (data.procSts!=null){
                html += '<tr><td class="td-label" align="center" colspan="4">应答信息</td></tr>'+
                    ...
                    '<td class="td-label-right" colspan="4" id="rjctInf">'+(data.rjctInf||'')+'</td>\n' +
                    '</tr>\n';
            }else {
