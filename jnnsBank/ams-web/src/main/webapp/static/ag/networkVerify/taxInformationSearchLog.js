...
layui.use(['form', 'layedit', 'laydate', 'loading', 'laytpl'], function () {
    ...
    $('#btn_add').on('click', function () {
        parent.tab.tabAdd({
            title: "纳税信息联网核查申请",
            href: 'networkVerify/taxInformation.html'
        });
    });
    ...
    $('#btn_feedback').on('click', function () {
        if (list.select(layerTips)) {
            ...
            parent.tab.tabAdd({
                title: "纳税信息疑义反馈申请",
                href: 'networkVerify/feedback.html?id='+id+'&type=taxInformation'
            });
        }
    });
    ...
    $('#btn_feedback_log').on('click', function () {
        parent.tab.tabAdd({
            title: "纳税信息疑义反馈记录",
            href: 'networkVerify/feedbackLog.html?type=taxInformation'
        });
    });
    ...
    function showTableHTML(id) {
        $.get(list.baseUrl+'/' + id, function (res) {
            ...
            html += '<tr><td class="td-label" colspan="6" align="center">请求信息</td></tr>'+
                ...
                '<td class="td-label-right" colspan="2">'+(data.coNm||'')+'</td>\n' +
                '<td class="td-label">统一社会信用代码：</td>\n' +
                '<td class="td-label-right" colspan="2">'+(data.uniSocCdtCd||'')+'</td>\n' +
                '</tr>\n' +
                '<tr>\n' +
                '<td class="td-label">纳税人识别号：</td>\n' +
                '<td class="td-label-right" colspan="2">'+(data.txpyrIdNb||'')+'</td>\n' +
                '<td class="td-label">操作员姓名：</td>\n' +
                '<td class="td-label-right" colspan="2">'+(data.opNm||'')+'</td>\n' +
                '</tr>\n' ;
