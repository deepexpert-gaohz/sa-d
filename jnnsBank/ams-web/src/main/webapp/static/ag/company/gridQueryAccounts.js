...
layui.use(['form', 'layedit', 'laydate'], function () {
    ...
    $.get('/apply/bank/count?timestamp=' + timestamp, function(dat) {
        var arr = dat.split(',');
        $('#dslCount').html(arr[0]);
        $('#yslCount').html(arr[1]);
        $('#slcgCount').html(arr[2]);
        $('#slsbCount').html(arr[3]);
    });

    $('#dsl').on('click', function () {
        parent.tab.tabAdd({
            href: '/ui/bank/pre.html',
            icon: 'fa fa-calendar-plus-o',
            title: '待受理'
        });
    });

    $('#ysl').on('click', function () {
        parent.tab.tabAdd({
            href: '/ui/bank/after.html',
            icon: 'fa fa-check',
            title: '已受理'
        });
    });

    $('#slcg').on('click', function () {
        parent.tab.tabAdd({
            href: '/ui/bank/success.html',
            icon: 'fa fa-calendar-check-o',
            title: '受理成功'
        });
    });

    $('#slsb').on('click', function () {
        parent.tab.tabAdd({
            href: '/ui/bank/fail.html',
