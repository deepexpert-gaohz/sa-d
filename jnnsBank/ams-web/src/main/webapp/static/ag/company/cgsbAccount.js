...
apply.queryParams = function (params) {
    var temp = {
        ...
        syncBeginDate: $.trim($("#syncBeginDate").val()),
        syncEndDate: $.trim($("#syncEndDate").val()),
...
apply.init = function () {
    ...
    apply.table = $('#' + apply.tableId).bootstrapTable({
        url: apply.baseUrl + '/list?whiteList=0', //请求后台的URL（*）  whiteList=0  表示进入不需要展现白名单数据的对公页面
        method: 'get', //请求方式（*）
    ...
    $('#btn_delete').on('click', function () {
        if (apply.select(layer)) {
            ...
            layer.confirm('确定删除数据吗？', null, function (index) {
                $.ajax({
                    type: "POST",
                    url: "../../allBillsPublic/deleteCgsb/" + billId,
                    dataType: "json",
                    success: function (data) {
                        if (data.rel) {//删除成功
                            layerTips.msg('删除成功');
