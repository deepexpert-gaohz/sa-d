...
layui.use(['form', 'layedit', 'laydate','layer','common'], function () {
    ...
    var laydateArr = beginEndDateInit2(laydate, 'syncBeginDate', 'syncEndDate', 'yyyy-MM-dd HH:mm:ss', true);
    ...
    $('#btn_query').on('click', function () {
        var queryParams = remind.queryParams();
        queryParams.pageNumber=1;
        remind.table.bootstrapTable('refresh', queryParams);
    });

    var remind = {
        baseUrl: "../../allBillsPublic",
        entity: "user",
        tableId: "bussinessTable",
        toolbarId: "toolbar",
        unique: "remindid",
        order: "desc",
        currentItem: {}
    ...
    remind.queryParams = function (params) {
        if (!params)
            return {
                ...
                pbcSyncMethod: $.trim($("#pbcSyncMethod").val()),
                pbcCheckStatus: $.trim($("#pbcCheckStatus").val()),
            };
    ...
    remind.init = function () {
        ...
        remind.table = $('#' + remind.tableId).bootstrapTable({
            url: remind.baseUrl + '/listForBills', //请求后台的URL（*）  whiteList=0  表示进入不需要展现白名单数据的对公页面
            method: 'get', //请求方式（*）
    ...
    remind.select = function (layerTips) {
        var rows = remind.table.bootstrapTable('getSelections');
        if (rows.length == 1) {
            remind.currentItem = rows[0];
            return true;
        } else {
            layerTips.msg("请选中一行");
            return false;
