...
authorityElement.queryParams = function (params) {
    if (!params)
        return {
            parentId: group.currentAuthorityMenu.id
        };
...
authorityElement.init = function () {
    ...
    $('#' + authorityElement.tableId).on('load-success.bs.table', function (data) {
        $.get(group.baseUrl + "/" + group.currentItem.id + "/authority/element", null, function (data) {
                if (data.code === 'ACK') {
                    authorityElement.table.bootstrapTable("checkBy", {field: "id", values: data.data});
                }
            }
        )
        ;
...
authorityElement.onCheckAll = function (rows) {
    ...
    $.ajax({
            url: group.baseUrl + "/" + group.currentItem.id + "/authority/element/adds",
            type: "POST",
            traditional: true,
            data: {
                menuId: menuId,
...
authorityElement.onUncheckAll = function (rows) {
    ...
    $.ajax({
            url: group.baseUrl + "/" + group.currentItem.id + "/authority/element/removes",
            type: "POST",
            traditional: true,
            data: {
                menuId: menuId,
