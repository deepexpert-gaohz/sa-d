...
public class ImportExcel {
	...
	public ImportExcel(String fileName, InputStream is, int headerNum, int sheetIndex) 
		...
		if (StringUtils.isBlank(fileName)){
			throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR, "导入文档为空!");
		}else if(fileName.toLowerCase().endsWith("xls")){
			this.wb = new HSSFWorkbook(is);    
        }else if(fileName.toLowerCase().endsWith("xlsx")){  
        	this.wb = new XSSFWorkbook(is);
        }else{
			throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR, "文档格式不正确!");
        }
		if (this.wb.getNumberOfSheets()<sheetIndex){
			throw new BizServiceException(EErrorCode.COMM_INTERNAL_ERROR, "文档中没有工作表!");
		}
	...
	public <E> List<E> getDataList(Class<E> cls, int... groups) throws InstantiationException, IllegalAccessException{
		...
		for (int i = this.getDataRowNum(); i <= this.getLastDataRowNum(); i++) {
			...
			for (Object[] os : annotationList){
				...
				Object val = this.getCellValue(row, ef.column());
				if (val != null){
					...
					try {
						if (valType == String.class){
							...
							if(val.toString().endsWith("%")){
								val = Double.valueOf(val.toString().substring(0, val.toString().length() - 1)) / 100d;
							}else{
